...
class Ebook(BaseModel):
    ...
    isbn = models.CharField(max_length=13, null=True, blank=True)
    category = models.ForeignKey(
        Category, on_delete=models.CASCADE, related_name="ebooks")
    ratings = models.FloatField(null=True, blank=True)
    file_format = models.CharField(max_length=50, null=True)  # Format of eBook (PDF, EPUB, etc.)
    ebook_file = models.FileField(upload_to='ebooks/', null=True, blank=True)  # Store the eBook file
...
class BookNote(BaseModel):
    ...
    def __str__(self):
        return f"{self.user.username}'s note for {self.book.name}"