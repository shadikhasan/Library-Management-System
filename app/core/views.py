...
def my_notebook(request):
    ...
    notes = BookNote.objects.filter(user=user)
    context = {
        'books': books,
        'notes': notes
    }
    return render(request, 'users/notebook.html', context)
...
def add_note(request):
    if request.method == 'POST':
        ...
        BookNote.objects.create(user=user, book=book, note=note_text)
        return redirect('my_notebook')
    else:
        pass
...
def edit_note(request, note_id):
    note = get_object_or_404(BookNote, pk=note_id)
    if request.method == 'POST':
    ...
    return render(request, 'users/edit_note.html', {'form': form})
...
def ebook_list(request):
    ...
    return render(request, 'users/ebook_list.html', {'ebooks': ebooks})

@login_required
def read_ebook(request, ebook_id):
    ebook = get_object_or_404(Ebook, pk=ebook_id)
    return render(request, 'users/read_ebook.html', {'ebook': ebook})
